# -*- coding: utf-8 -*-
"""
Good News Agent - reliable RSS fetch
- Fetches news from multiple reliable RSS feeds
- No sentiment filter, all items are included
- Translates titles + summaries to Hebrew
- Skips silently feeds that are unavailable or empty
- Timeout extended to 25 seconds
"""
...
def build_html_digest(articles):
    ...
    parts = [
        ...
        f"</head><body><div class='container'><header><h1>Good News Digest</h1><div class='lead'>עדכון — {now} | חדשות מהעולם בתרגום לעברית</div></header>",
...
def send_email(html):
    ...
    try:
        ...
        msg.attach(MIMEText("מצורף דוח החדשות להיום (לינקים ותקצירים).", "plain", "utf-8"))
